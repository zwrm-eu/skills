...
- **Interactive workspace**: `zwrm agent claude` attaches you to a persistent dev VM running Claude Code (or `codex`).
...
zwrm agent run — Start an agent run (a prompt on one of your agents)
      ...
      --model string       Claude model: opus or sonnet (default: agent default)
...
zwrm agent run continue <run-id> — Continue a finished run's conversation with a follow-up run
      ...
      --model string       Claude model: opus or sonnet (default: the run being continued)
...
zwrm agent update <instance-or-id> — Update an agent's defaults (size, model, effort, instructions, budget, TTL, scopes)
      ...
      --model string             Default model: opus or sonnet
